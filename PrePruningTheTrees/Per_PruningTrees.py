...
#构建决策树核心
def createTree(dataSet,attributes,returnAssMax_Validation):
    ...
    temp_labels=copy.deepcopy(attributes) 
    ...
    for value in uniqueVals:
        ...
        myTree[bestFeatLabel][value] = createTree(splitDataSet(dataSet, bestFeat, value),subAttributes,returnAssMax_Validation) # 递归创建树的节点(对每一个分支)
    
    if testing(myTree,returnAssMax_Validation,temp_labels) < testingMajor(majorityCnt(classList),returnAssMax_Validation):  
        return myTree  
...
def rungo():   #运行
    ...
    myTree = createTree(returnAssMax,attributes,returnAssMax_Validation)
    print (myTree)
    #保存决策树
